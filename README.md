...
```swift
dependencies: [
    .package(url: "https://github.com/swift-molecules/swift-initialization.git", branch: "main")
]
```
...
.target(
    name: "App",
    dependencies: [
        .product(name: "Initialization", package: "swift-initialization"),
    ]
)
```
...
The package follows the ecosystem's operation-domain layout (the same one `swift-iterator` uses), decomposed into focused products with zero external dependencies. Import the umbrella for everything, or a single leaf product for a minimal surface.

| Product | When to import |
|---------|----------------|
| `Initialization` | The umbrella — the whole domain. Resolves `import Initialization` to the namespace, the active `Initializing` protocol, the `Initialization.Witness`, and the passive `Initiable`. |
...
| `Initialization Standard Library Integration` | Opt-in `Initiable` conformances for the stdlib growable disciplines (`Array`, `ContiguousArray`, `ArraySlice`, `Set`, `Dictionary`, `String`, `Substring`). Kept out of the umbrella so consumers choose the retroactive conformances. |
